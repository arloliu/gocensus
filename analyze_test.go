...
func TestAnalyzeReportsScope(t *testing.T) {
	...
	if result.Scope != "production excludes generated and mock files; testdata directories excluded" {
	...
	if result.Scope != "production includes generated and mock files; testdata directories excluded" {
		t.Fatalf("Scope = %q, want inclusion scope", result.Scope)
	}
}

func TestAnalyzeCanIncludeTestdata(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, dir, "go.mod", "module example.com/app\n\ngo 1.21\n")
	writeFile(t, dir, "main.go", "package main\n")
	writeFile(t, dir, "testdata/fixture.go", "package fixture\n")
	...
	if result.Files.Total != 2 || result.Files.Production != 2 {
		t.Fatalf("files = %#v, want main.go and testdata/fixture.go counted as production", result.Files)
	}
	if result.Scope != "production excludes generated and mock files; testdata directories included" {
		t.Fatalf("Scope = %q, want testdata inclusion scope", result.Scope)
	}
}
